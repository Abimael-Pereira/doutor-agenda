...
interface UpsertAppointmentFormProps {
  ...
  onSuccess?: () => void;
  defaultValues?: Partial<UpsertAppointmentSchema>;
}
...
export function UpsertAppointmentForm({
  ...
  const form = useForm<UpsertAppointmentSchema>({
    ...
    defaultValues: {
      ...
      appointmentPriceInCents: defaultValues?.appointmentPriceInCents || 0,
      ...defaultValues,
    },
  });

  const { executeAsync, isExecuting } = useAction(upsertAppointment, {
    onSuccess: ({ data }) => {
  ...
  const onSubmit = async (data: UpsertAppointmentSchema) => {
